...
| 99 FX A/B programs | Compact FX rows | `fx-program-bank`, `fx-unit-*`, `sync-mixer-graph` | `FxProgramRegistry`, `FxProgramController`, `FxProgramWetProcessor`, `MixerGraph` | FX unit snapshot | `FxProgramRegistry*Test`, `FxProgramControllerTest`, `FxProgramWetProcessorTest`, `MixerGraphTest`, `EngineGraphSyncJsonTest` | UI rows call ACK commands, FX send/return state reaches native graph, and factory recipes have native wet processors; full 99-program auditory QA and live callback integration remain pending | `PARTIAL` |
