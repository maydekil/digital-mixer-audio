...
| 99 FX A/B programs | Compact FX rows | `fx-program-bank`, `fx-unit-*`, `sync-mixer-graph` | `FxProgramRegistry`, `FxProgramController`, `FxProgramWetProcessor`, `MixerRenderRuntime`, `MixerGraph` | FX unit snapshot | `FxProgramRegistry*Test`, `FxProgramControllerTest`, `FxProgramWetProcessorTest`, `MixerRenderRuntimeTest`, `MixerGraphTest`, `EngineGraphSyncJsonTest` | UI rows call ACK commands, FX send/return state reaches native graph, factory recipes have native wet processors, and a render runtime binds default FX A/B programs to graph processing; full 99-program auditory QA and desktop live callback integration remain pending | `PARTIAL` |
