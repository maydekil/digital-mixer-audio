...
| 99 FX A/B programs | Compact FX rows | `fx-program-bank`, `fx-unit-*`, `sync-mixer-graph` | `FxProgramRegistry`, `FxProgramController`, `MixerGraph` | FX unit snapshot | `FxProgramRegistry*Test`, `FxProgramControllerTest`, `MixerGraphTest`, `EngineGraphSyncJsonTest` | UI rows call ACK commands and FX send/return state reaches native graph; final program-specific DSP rendering and 99-program auditory QA pending | `PARTIAL` |
