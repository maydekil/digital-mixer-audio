...
| EQ/HPF/LPF/noise/compressor/de-esser | Channel Processing panel, strip shortcuts | preview adapter; native DSP modules | `Eq`, `Dynamics`, `DeEsser` | channel processor state | `EqTest`, `DynamicsTest`, `DeEsserTest` | DSP tests pass; realtime insert graph wiring still partial | `PARTIAL` |
