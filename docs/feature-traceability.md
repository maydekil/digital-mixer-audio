...
| record dry/processed/master | REC buttons, global REC preview | recording module contract | `Recording` | recording take metadata | `RecordingTest` | writer tests pass and take metadata distinguishes dry/processed/master replay semantics; live record/replay journey pending INT-01 | `PARTIAL` |
| codecs/export/stems | Export workflow pending | export module contract | `Export`, `MixerRenderRuntime`, codec probe script | export job/session fields | `ExportTest` | codec probe and native tests exist; processed graph export now renders through the mixer runtime; packaged import/export not run | `PARTIAL` |
