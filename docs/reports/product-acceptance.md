...
| E2E-05 | Record dry/master then replay | `PARTIAL` | Native take metadata and desktop recording preflight/start/stop foundation exist; saved non-partial takes are inserted back into the mixer as replay/export source channels; live PCM writing and alignment are not exposed end-to-end. |
| E2E-06 | Processed vocal take replay | `PARTIAL` | Processed take metadata can request neutral insert replay, persists in session JSON, and can become a vocal replay source channel after stop; successful live processed recording flow not run. |
| E2E-07 | Save/open/collect/relink | `PARTIAL` | Session contracts cover channel state, FX, harmony, plugin, and take metadata; desktop Open/Save Project can roundtrip renderer snapshot `.lam.json`; Electron autosave writes validated project JSON with previous-good backup and startup restore prompt; Collect copies present media and Open/Collect can prompt for missing replacements through native file dialogs and write back updated session JSON; packaged/manual relink/recovery acceptance pending. |
| E2E-08 | Master + FX return stems | `PARTIAL` | Stem planning tests and desktop/native export preflight pass; native WAV render smoke exists for the master path with explicit tail and partial cancel behavior; rendered FX return stem files not inspected. |
...
- Desktop recording preflight/start/stop can choose a take folder, ask native for take metadata, create/finalize a WAV take container, persist takes, and insert saved non-partial takes as replay source channels, but callback-driven live PCM writing and latency alignment remain pending.
- Project collect/relink/autosave has automated file-level and renderer workflow coverage, but packaged manual acceptance remains pending.
